//
//  Pocket SDR - Pocket SDR FE 4CH firmware for EZ-USB FX3
//
//  References:
//
...
//
//  Options:
//  -DREV_A    For H/W rev.A (separated SDATA lines)
//
//  Author:
//  T.TAKASU
//
//  History:
//  2024-04-10  1.0  new
//  2024-05-13  1.1  support H/W rev.A with -DREV_A
//
#include <stdio.h>
#include <stdint.h>
#include "cyu3types.h"
...
#define LOCK_C       38         // FX3 GPIO(38) <-- MAX2771 CH3 LD  (0x26)
#define LOCK_D       39         // FX3 GPIO(39) <-- MAX2771 CH4 LD  (0x27)
#define CSN_A        40         // FX3 GPIO(40) --> MAX2771 CH1 CSN (0x28)
#define CSN_B        41         // FX3 GPIO(41) --> MAX2771 CH2 CSN (0x29)
#define CSN_C        42         // FX3 GPIO(42) --> MAX2771 CH3 CSN (0x2A)
#define CSN_D        43         // FX3 GPIO(43) --> MAX2771 CH4 CSN (0x2B)
#define RF_SW_A      44         // FX3 GPIO(44) --> RF_SW CH1       (0x2C)
#define RF_SW_B      45         // FX3 GPIO(45) --> RF_SW CH2       (0x2D)
#define RF_SW_C      46         // FX3 GPIO(46) --> RF_SW CH3       (0x2E)
#define RF_SW_D      47         // FX3 GPIO(47) --> RF_SW CH4       (0x2F)
#define LED1         48         // FX3 GPIO(48) --> LED1            (0x30)
#define LED2         49         // FX3 GPIO(49) --> LED2            (0x31)
#define USB3_PORT_SEL 50        // FX3 GPIO(50) --> USB3_PORT_SEL   (0x32)
#define SCLK         53         // FX3 GPIO(53) --> MAX2771 SCLK    (0x35)
#ifdef REV_A
#define SDATA_A      54         // FX3 GPIO(54) <-> MAX2771 CH1 SDATA (0x36)
#define SDATA_B      55         // FX3 GPIO(55) <-> MAX2771 CH2 SDATA (0x37)
#define SDATA_C      56         // FX3 GPIO(56) <-> MAX2771 CH3 SDATA (0x38)
#define SDATA_D      57         // FX3 GPIO(57) <-> MAX2771 CH4 SDATA (0x39)
#else
#define SDATA        55         // FX3 GPIO(55) <-> MAX2771 SDATA   (0x37)
#endif
...
#define EE_ADDR_H    0xFF00     // EEPROM MAX2771 settings header address
#define EE_ADDR_S    0xFF04     // EEPROM MAX2771 settings address
#define HEAD_REG     0xABC00CBA // MAX2771 settings header
#define MAX_CH       4          // Number of MAX2771 channels
...
static const uint8_t port_out[] = {RF_SW_A, RF_SW_B, RF_SW_C, RF_SW_D,
    USB3_PORT_SEL, SCLK, CSN_A, CSN_B, CSN_C, CSN_D, LED1, LED2, 0};
#ifdef REV_A
static const uint8_t port_inp[] = {LOCK_A, LOCK_B, LOCK_C, LOCK_D, SDATA_A,
    SDATA_B, SDATA_C, SDATA_D, 0};
static const uint8_t port_sdata[] = {SDATA_A, SDATA_B, SDATA_C, SDATA_D};
#else
static const uint8_t port_inp[] = {LOCK_A, LOCK_B, LOCK_C, LOCK_D, SDATA, 0};
static const uint8_t port_sdata[] = {SDATA, SDATA, SDATA, SDATA};
#endif
static const uint8_t port_csn[MAX_CH] = {CSN_A, CSN_B, CSN_C, CSN_D};
static const uint8_t port_rsw[MAX_CH] = {RF_SW_A, RF_SW_B, RF_SW_C, RF_SW_D};
...
static const uint32_t reg_default[MAX_CH][MAX_ADDR] = {
    ...
    {0xA224A011, 0x28550288, 0x0EAF31D0, 0x198C0008, 0x027F6320, 0x00000070,
     0x08000000, 0x10000002, 0x01E0F401, 0x00000002, 0x00000004},
    {0xA224A03D, 0x28550288, 0x0EAF31D0, 0x198C0008, 0x03D46500, 0x00000070,
     0x08000000, 0x10000002, 0x01E0F401, 0x00000002, 0x00000004},
    {0xA224A00D, 0x28550288, 0x0EAF31D0, 0x198C0008, 0x00D52100, 0x00000070,
     0x08000000, 0x10000002, 0x01E0F401, 0x00000002, 0x00000004}
};
...
static int write_reg(uint8_t ch, uint8_t addr, uint32_t val)
{
    ...
    // sync RF switch for MAX2771 LNA
    if (addr == 0x0) {
        uint8_t lna_mode = (val >> 15) & 3; // LNAMODE (0:high act,1:low act)
        write_iop(port_rsw[ch], lna_mode == 1);
        spi_delay();
    }
    return 1;
}
...
static int handle_req(uint8_t req, uint16_t val, uint16_t len)
{
    ...
    if (req == VR_STAT) {
        uint8_t stat = (app_act << 5) + (bulk_act << 4) +
            (read_iop(LOCK_A) << 3) + (read_iop(LOCK_B) << 2) +
            (read_iop(LOCK_C) << 1) + read_iop(LOCK_D);
        EP0BUFF[0] = VER_FW;
...
static int app_init(void)
{
    ...
    CyU3PI2cConfig_t icfg = {0};
    icfg.bitRate = I2C_BITRATE;
    icfg.busTimeout = 0xFFFFFFFF; // no timeout
    icfg.dmaTimeout = 0xFFFF;     // no timeout
    if (CyU3PI2cSetConfig(&icfg, NULL)) return 0;
...
int main(void)
{
    ...
    // IO port mask
    uint64_t mask = 0;
    for (int i = 0; port_inp[i]; i++) mask |= (uint64_t)1 << port_inp[i];
    for (int i = 0; port_out[i]; i++) mask |= (uint64_t)1 << port_out[i];
