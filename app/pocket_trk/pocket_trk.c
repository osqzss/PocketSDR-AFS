...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-sig") && i + 1 < argc) {
            ...
            paths[2] = argv[++i]; // TEB: 2025-03-14 Issue #32
        }
        else if (!strcmp(argv[i], "-nmea") && i + 1 < argc) {
            paths[0] = argv[++i]; // TEB: 2025-03-14 Issue #32
        }
        else if (!strcmp(argv[i], "-rtcm") && i + 1 < argc) {
            paths[1] = argv[++i]; // TEB: 2025-03-14 Issue #32
        }
    ...
    // TEB: 2025-03-11 Set Q sign flag
    //if ((!strcmp(sig, "AFSD"))||(!strcmp(sig, "AFSP"))) {
    if (!inv_q) {
        rcv->qsign = SDR_QSIGN_NOM;
    }
    else { // Inverse Q sign for MAX2771
        rcv->qsign = SDR_QSIGN_INV;
    }
