...
// TEB: Satellite code for LANS AFS
// L = carrier, 9 = S/2492.028MHz, D = data, P = pilot
#define CODE_L9D 80
#define CODE_L9P 81
...
static uint8_t sig2code(const char *sig)
{
    ...
    static const char *sigs[] = {
        ...
        "B2AP" , "B2BI" , "B3I"  , "I1SD" , "I1SP" , "I5S"  , "ISS"  , 
        "AFSD" , "AFSP" , NULL
    };
...
// TEB: Add observation code for LANS AFS
static char *afsobs[] = {(char*)"9D",(char*)"9P"}; // 9 = S/2492.028MHz, D = data, P = pilot
...
static void out_log_obs(double time, const obs_t *obs)
{
    for (int i = 0; i < obs->n; i++) {
        ...
        for (int j = 0; j < NFREQ + NEXOBS; j++) {
            ...
            // TEB: Convert code number to observation code
            if (data->code[j] >= CODE_L9D) {
                obscode = (data->code[j]==CODE_L9D? afsobs[0]:afsobs[1]);
            }
            else {
                obscode = code2obs(data->code[j]);
            }
            sdr_log(3, "$OBS,%.3f,%d,%.3f,%s,%s,%.1f,%.3f,%.3f,%.3f,%d", time,
                week, tow, sat, obscode,
...
{    
    // TEB: Data index for LANS AFS
    if (code >= CODE_L9D) {
        return (code==CODE_L9D? 0:1);
    }
    else {
...
static double gen_prng(gtime_t time, const sdr_ch_t *ch)
{
    ...
    /*
    sdr_log(3,"$DBG,%s,%-5s,%3d,%4d,%10.3f,%10.3f,%12.9f,%12.9f,%12.3f", ch->sat, ch->sig,
        ch->prn, ch->week, tow, ch->tow * 1e-3, ch->coff, tau, CLIGHT * tau);
    */
#endif
    return CLIGHT * tau;
}
...
void sdr_pvt_udnav(sdr_pvt_t *pvt, sdr_ch_t *ch)
{
    ...
    else if (!strcmp(ch->sig, "AFSD")) { // TEB: AFSD NAV
        if (ch->nav->type == 2 && decode_afs_frame(data, ch->prn)) {
...
void sdr_pvt_udsol(sdr_pvt_t *pvt, int64_t ix)
{
    ...
    if (pvt->ix > 0 && (pvt->nch >= pvt->rcv->nch ||
        ...
        // TEB: PVT for LANS AFS
        int pvt_afs = 0;

        for (int i = 0; i < pvt->obs->n; i++) {
            if (pvt->obs->data[i].code[0] == CODE_L9D) {
                pvt_afs = 1;
                break;
            }
        }
...
void sdr_pvt_solstr(sdr_pvt_t *pvt, char *buff)
{
    ...
    if (norm(pvt->sol->rr, 3) > 1e-6) {
        ...
        stat = pvt->sol->stat;
        if (stat > MAXSOLQ)
            xyz2llh(pvt->sol->rr, pos); // TEB: for LANS AFS positioning
        else
            ecef2pos(pvt->sol->rr, pos);
    }
    else {
