...
// TEB: Support non-inverted Q sign
//static void gen_LUT(sdr_buff_t **buff, int nbuff, sdr_cpx8_t LUT[][256])
static void gen_LUT(sdr_rcv_t *rcv, sdr_cpx8_t LUT[][256])
{
    //static const int8_t valI[] = {1, 3, -1, -3}, valQ[] = {-1, -3, 1, 3};
    static const int8_t val[] = {1, 3, -1, -3}, val_inv[] = {-1, -3, 1, 3};
    ...
    for (int i = 0; i < 256; i++) {
        ...
        I[0] = val[(i>>0) & 0x3]; I[1] = val[(i>>4) & 0x3];
        if (rcv->qsign == SDR_QSIGN_INV) {
            Q[0] = val_inv[(i>>2) & 0x3]; Q[1] = val_inv[(i>>6) & 0x3];
        }
        else {
            Q[0] = val[(i>>2) & 0x3]; Q[1] = val[(i>>6) & 0x3];
        }
...
static void write_buff(sdr_rcv_t *rcv, const uint8_t *raw, int64_t ix)
{
    ...
    if (rcv->fmt == SDR_FMT_INT8) { // int8
        ...
        for (int j = 0; j < rcv->N * 2; i++, j += 2) {
            // TEB: Inverted Q sign for MAX2711
            if (rcv->qsign == SDR_QSIGN_INV) {
